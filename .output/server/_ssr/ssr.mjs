...
async function getStartManifest(matchedRoutes) {
	const { tsrStartManifest } = await import("../_tanstack-start-manifest_v-CBLMakCk.mjs");
...
async function loadEntries() {
	const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
		import("./router-D3cy_axw.mjs").then((n) => n.router_exports),
		import("./start-Bzdcyk0a.mjs"),
...
var extractLocaleFromRequestWithStrategies = (request, strategies, url = request.url) => {
	...
	for (const strat of strategies) {
		if (strat === "url") locale = extractLocaleFromUrl(effectiveRequestUrl);
...
function extractLocaleFromUrl(url) {
	...
	/** @type {Locale | undefined} */
	let result;
	result = defaultUrlPatternExtractLocale(normalizeTrailingSlash(typeof url === "string" ? new URL(url, "http://example.com") : new URL(url)));
	cachedUrl = urlString;
	cachedLocale = result;
	return result;
}
...
function localizeUrlDefaultPattern(url, locale) {
	...
	if (extractLocaleFromUrl(urlObj) === locale) return normalizeTrailingSlash(urlObj);
...
var server_default = { fetch(req) {
	return paraglideMiddleware(req, () => server_default$1.fetch(req));
} };
//#endregion
export { deLocalizeUrl, server_default as default, getLocale, locales, localizeUrl, setLocale, ssr_exports };
