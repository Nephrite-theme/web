//#region node_modules/.pnpm/gsap@3.15.0/node_modules/gsap/ScrollSmoother.js
...
var _maxScroll$1 = function _maxScroll(scroller) {
...
var ScrollSmoother = /*#__PURE__*/ function() {
	function ScrollSmoother(vars) {
		var _this = this;
		_coreInitted$2 || ScrollSmoother.register(gsap$2) || console.warn("Please gsap.registerPlugin(ScrollSmoother)");
		vars = this.vars = vars || {};
		_mainInstance && _mainInstance.kill();
		_mainInstance = this;
		_context$2(this);
		var _vars = vars, smoothTouch = _vars.smoothTouch, _onUpdate = _vars.onUpdate, onStop = _vars.onStop, smooth = _vars.smooth, onFocusIn = _vars.onFocusIn, normalizeScroll = _vars.normalizeScroll, wholePixels = _vars.wholePixels, content, wrapper, height, mainST, effects, sections, intervalID, wrapperCSS, contentCSS, paused, pausedNormalizer, recordedRefreshScroll, recordedRefreshScrub, allowUpdates, self = this, effectsPrefix = vars.effectsPrefix || "", scrollFunc = ScrollTrigger$2.getScrollFunc(_win$2), smoothDuration = ScrollTrigger$2.isTouch === 1 ? smoothTouch === true ? .8 : parseFloat(smoothTouch) || 0 : smooth === 0 || smooth === false ? 0 : parseFloat(smooth) || .8, speed = smoothDuration && +vars.speed || 1, currentY = 0, delta = 0, startupPhase = 1, tracker = _getVelocityProp$1(0), updateVelocity = function updateVelocity() {
			...
			if (y !== currentY && !paused || force) {
				...
				delta = y - currentY;
				currentY = y;
				ScrollTrigger$2.isUpdating || ScrollSmoother.isRefreshing || ScrollTrigger$2.update();
			}
			...
			if (arguments.length) {
				...
				ScrollTrigger$2.isRefreshing ? mainST.update() : scrollFunc(value / speed);
				return this;
			}
			return -currentY;
			...
			var ratio = isNaN(st.ratio) ? 1 : st.ratio, change = st.end - st.start, distance = position - st.start, offset = st.offset || 0, pins = st.pins || [], pinOffset = pins.offset || 0, progressOffset = st._startClamp && st.start <= 0 || st.pins && st.pins.offset ? 0 : st._endClamp && st.end === _maxScroll$1() ? 1 : .5;
			...
			var pins = st.pins, markers = st.markers, dif, isClamped, start, end, nativeStart, nativeEnd, i = 0, trig;
			...
			if (effects) {
				...
				effects.forEach(function(st) {
					var start = st._startClamp || st.start, end = st.autoSpeed ? Math.min(_maxScroll$1(), st.end) : start + Math.abs((st.end - start) / st.ratio), offset = end - st.end;
					start -= offset / 2;
					end -= offset / 2;
					if (start > end) {
						var s = start;
			...
			var getSpeed = effectValueGetter("speed", speed, index, el), getLag = effectValueGetter("lag", lag, index, el), startY = gsap$2.getProperty(el, "y"), cache = el._gsap, ratio, st, autoSpeed, scrub, progressOffset, yOffset, pins = [], initDynamicValues = function initDynamicValues() {
				speed = getSpeed();
				lag = parseFloat(getLag().value);
				ratio = parseFloat(speed.value) || 1;
				autoSpeed = speed.value === "auto";
				progressOffset = autoSpeed || st && st._startClamp && st.start <= 0 || pins.offset ? 0 : st && st._endClamp && st.end === _maxScroll$1() ? 1 : .5;
				scrub && scrub.kill();
		...
		this.offset = function(target, position, ignoreSpeed) {
			...
			var cssText = target.style.cssText, st = ScrollTrigger$2.create({
				trigger: target,
				start: position || "top top"
			}), y;
			if (effects) startupPhase ? ScrollTrigger$2.refresh() : adjustParallaxPosition([st], true);
		...
		this.effects = function(targets, config) {
			...
			config = config || {};
			var _config = config, speed = _config.speed, lag = _config.lag, effectsPadding = _config.effectsPadding, effectsToAdd = [], i = 0, st;
			for (; i < targets.length; i++) {
				st = createEffect(targets[i], speed, lag, i, effectsPadding);
				st && effectsToAdd.push(st);
			}
			(_effects = effects).push.apply(_effects, effectsToAdd);
			config.refresh !== false && ScrollTrigger$2.refresh();
			return effectsToAdd;
		};
		...
		ScrollTrigger$2.defaults({ scroller: wrapper });
		...
		mainST = ScrollTrigger$2.create({
			...
			onRefreshInit: function onRefreshInit(self) {
				...
				scroll.y = 0;
				if (smoothDuration) {
					ScrollTrigger$2.isTouch === 1 && (wrapper.style.position = "absolute");
					wrapper.scrollTop = 0;
			...
			if (arguments.length) {
				if (!!paused !== value) {
					if (value) {
						...
						paused.nested = _inputObserver$1(_docEl$2, "wheel,touch,scroll", true, allowNestedScroll !== false);
					} else {
						paused.nested.kill();
						paused.kill();
						paused = 0;
		...
		this.kill = this.revert = function() {
			...
			defaults && defaults.scroller === wrapper && ScrollTrigger$2.defaults({ scroller: _win$2 });
		...
		if (normalizeScroll) this.normalizer = ScrollTrigger$2.normalizeScroll(normalizeScroll === true ? {
			debounce: true,
			content: !smoothDuration && content
		} : normalizeScroll);
		ScrollTrigger$2.config(vars);
		"scrollBehavior" in _win$2.getComputedStyle(_body$2) && gsap$2.set([_body$2, _docEl$2], { scrollBehavior: "auto" });
	...
	ScrollSmoother.register = function register(core) {
		if (!_coreInitted$2) {
			...
			if (gsap$2) {
				...
				gsap$2.core.globals("ScrollSmoother", ScrollSmoother);
				if (_body$2 && ScrollTrigger$2) {
					_onResizeDelayedCall = gsap$2.delayedCall(.2, function() {
						return ScrollTrigger$2.isRefreshing || _mainInstance && _mainInstance.refresh();
					}).pause();
